...
const CreateRouteModal = ({ isOpen, onClose, onSave, allStops }) => {
  ...
  // Ensure start(id=0) first and end(id=1) last if available
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Preview route through all segments
  useEffect(() => {
  ...
  // Recompute distance/ETA when stops change
  useEffect(() => { computeDistanceAndEta(selectedStops); }, [selectedStops]);
  ...
  async function computeDistanceAndEta(stops) {
    try {
      ...
      for (const s of stops) {
        if (s.vi_do == null || s.kinh_do == null || !isFinite(Number(s.vi_do)) || !isFinite(Number(s.kinh_do))) return;
      }
      const toMeters = (distance) => {
        if (typeof distance === 'number' && isFinite(distance)) return distance;
        if (distance && typeof distance.value === 'number' && isFinite(distance.value)) return distance.value;
        if (typeof distance === 'string') {
          const km = distance.match(/([0-9]+(?:\.[0-9]+)?)\s*km/i); if (km) return Math.round(parseFloat(km[1]) * 1000);
          const m = distance.match(/([0-9]+)\s*m(?!in)/i); if (m) return parseInt(m[1], 10);
        }
        return 0;
      };
      const toSeconds = (duration) => {
        if (typeof duration === 'number' && isFinite(duration)) return duration;
        if (duration && typeof duration.value === 'number' && isFinite(duration.value)) return duration.value;
        if (typeof duration === 'string') {
          let total = 0; const h = duration.match(/([0-9]+)\s*h/i); const min = duration.match(/([0-9]+)\s*m/i); const s = duration.match(/([0-9]+)\s*s/i);
          if (h) total += parseInt(h[1], 10) * 3600; if (min) total += parseInt(min[1], 10) * 60; if (s) total += parseInt(s[1], 10);
      ...
      for (let i = 0; i < stops.length - 1; i++) {
        const o = `${stops[i].vi_do},${stops[i].kinh_do}`; const d = `${stops[i + 1].vi_do},${stops[i + 1].kinh_do}`;
        if (!o || !d || o === d) continue;
        let url = `https://rsapi.goong.io/Direction?origin=${encodeURIComponent(o)}&destination=${encodeURIComponent(d)}&vehicle=car&api_key=${import.meta.env.VITE_GOONG_API_KEY}`;
        const resp = await fetch(url); const data = await resp.json(); const route = data?.routes?.[0]; const legs = Array.isArray(route?.legs) ? route.legs : [];
        if (legs.length > 0) { legs.forEach(l => { totalDistance += toMeters(l.distance); totalDuration += toSeconds(l.duration); }); }
        else if (route) { totalDistance += toMeters(route.distance); totalDuration += toSeconds(route.duration); }
      }
      ...
      setRouteDistance(meters); setRouteEta(minutes);
    } catch {}
  }
  ...
  const handleCreate = async () => {
    ...
    const payload = {
      ten_tuyen_duong: base.ten_tuyen_duong,
      quang_duong: base.quang_duong,
      thoi_gian_du_kien: base.thoi_gian_du_kien,
      mo_ta: base.mo_ta,
      tuyen_duong_diem_dung,
    };
    try {
      const res = await TuyenDuongService.createTuyenDuong(payload);
      if (res && res.success) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center mb-4 pb-4 border-b">
          <h2 className="text-2xl font-bold text-gray-800">Tạo tuyến đường</h2>
                ...
                {selectedStops.map((stop, index) => (
                  <div key={stop.id_diem_dung} className="flex justify-between items-center bg-white p-2 rounded shadow-sm border">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => moveStop(index, -1)} disabled={index === 0 || Number(stop.id_diem_dung) === 0 || Number(stop.id_diem_dung) === 1} className="p-1 rounded-md hover:bg-gray-200 disabled:opacity-30 disabled:cursor-not-allowed"><FaArrowUp/></button>
                      <button onClick={() => moveStop(index, 1)} disabled={index === selectedStops.length - 1 || Number(stop.id_diem_dung) === 0 || Number(stop.id_diem_dung) === 1} className="p-1 rounded-md hover:bg-gray-200 disabled:opacity-30 disabled:cursor-not-allowed"><FaArrowDown/></button>
                    </div>
                  </div>
                ))}
