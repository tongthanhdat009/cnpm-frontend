...
// --- COMPONENT MODAL ĐỂ THÊM/SỬA PHỤ HUYNH ---
const ParentModal = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = (event) => {
    ...
    const newParentData = {
      // id_nguoi_dung sẽ được tạo tự động ở backend
      ho_ten: formData.get('ho_ten'),
      email: formData.get('email'),
      so_dien_thoai: formData.get('so_dien_thoai'),
      // Mật khẩu cần được xử lý cẩn thận hơn trong thực tế
      mat_khau_bam: formData.get('mat_khau'), 
      vai_tro: 'phu_huynh',
    };
    onSave(newParentData);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-lg">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Thêm phụ huynh mới</h2>
